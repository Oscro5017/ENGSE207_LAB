...
test_endpoint() {
    ...
    if [ "$method" = "GET" ]; then
        ...
        response=$(curl -s $curl_opts -X POST -H "Content-Type: application/json" -d "$data" "$url")
    elif [ "$method" = "PATCH" ]; then
        response=$(curl -s $curl_opts -X PATCH -H "Content-Type: application/json" -d "$data" "$url")
